...
export const podcastRepository = (client: Http): PodcastRepository => ({
  ...
  getPodcastDetail: async (id: string | null) => {
    ...
    const response = await client.get<PodcastDetailDTO>(
      `${CORS}`
      //https://api.allorigins.win/get?url=https://itunes.apple.com/lookup?id=1523872673
    );
    ...
    console.log(parsedPodcastDetail, "PARSED")
    if (typeof window !== "undefined") {
      console.log(
        "[HYDATE.fullfilled]: getPodcastDetail/%s",
        response
      );
    } else {
      console.log(
